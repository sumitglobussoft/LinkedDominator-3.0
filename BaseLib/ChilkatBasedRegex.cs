...
namespace BaseLib
{
    public static class ChilkatBasedRegex
    {
        ...
        public static List<string> GettingAllUrls1_writtenBysharan_fullUrl(string pageSource)
        {
            List<string> suburllist1 = new List<string>();

            try
            {
                string[] arr = Regex.Split(pageSource, "link_nprofile_view_4");
                foreach (string item in arr)
                {
                    if (!item.Contains("<!DOCTYPE"))
                    {
                        try
                        {
                            ...
                            string url = Utils.getBetween(item, "\":\"", "\"");
                            if (url.Contains("\u002d"))
                            {
                                url = url.Replace("\u002d", "-");
                            }
